...
export default function RegistrationsClient() {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        if (data.events) {
          const sortedEvents = data.events.sort((a, b) => new Date(a.event_date) - new Date(b.event_date));
  ...
  useEffect(() => {
    ...
    const fetchEventDetails = async () => {
      try {
        ...
        const data = await response.json();
        if (data.event) setDetail(data.event);
      } catch {
  ...
  const handleSubmit = async e => {
    ...
    if (!selectedId || !form.full_name || !form.email) {
      setMsg(m => ({ ...m, error: 'กรุณาเลือกกิจกรรม และกรอกชื่อ + อีเมล' }));
      setSubmitting(false);
      return;
    }
    ...
    try {
      ...
      if (data.message) {
        ...
        const refreshed = await fetch(`/api/events/${selectedId}`);
        const updated = await refreshed.json();
        if (updated.event) setDetail(updated.event);
      } else {
        setMsg({ error: data.error || 'เกิดข้อผิดพลาดในการลงทะเบียน' });
      }
    } catch {
      setMsg({ error: 'เกิดข้อผิดพลาดในการเชื่อมต่อกับเซิร์ฟเวอร์' });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const max = detail?.max_attendees || 0;
    ...
    <div className="flex flex-col min-h-screen font-kanit bg-gray-50">
      ...
      <main className="flex-grow py-8 md:py-12">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <RegistrationHeader />
          <RegistrationStatus error={msg.error} success={msg.success} />
          <EventSelector events={events} selectedId={selectedId} onSelect={handleSelect} loading={loading} />
          {detail && (
            <>
